...
const QrCodeReader: NextPage = () => {
        ...
        return <div style={{
            ...
            <Typography variant="h6" style={{marginBottom: "1em", width: "90%", maxWidth: "1000px"}} fontWeight="bold"
                        align="center">
                QRコードを読み込んでください
            </Typography>
            {
                ...
                >
                    {devices.map((device, index) => <MenuItem value={device.id}
                                                              key={index.toString()}>{device.name}</MenuItem>)}
                </Select>
            }
            ...
            <video
                ...
                ref={videoRef}/>
            <Link href={"/users/orders"} passHref>QRリーダーを終了する</Link>
        </div>;
    }
;
